...
public function track()
    {
        ...
        // Underlying history (for timeframe price display)
        $underlyingMap = [];
        $underlyingTsMap = [];
        foreach ($lookbacks as $m) {
            ...
            $tsRow = $db->query("
                ...
                FROM oi_snapshots
                WHERE symbol=? AND expiry=? AND ts<=?
            ", [$symbol,$expiry,$cutoff])->getRowArray();
            $tsPrev = $tsRow['ts'] ?? null;
            if ($tsPrev) {
                $uRow = $db->query("
                    SELECT underlying, ts
                    FROM oi_snapshots
                    WHERE symbol=? AND expiry=? AND ts=?
                    LIMIT 1
                ", [$symbol,$expiry,$tsPrev])->getRowArray();
                if ($uRow && isset($uRow['underlying'])) {
                    $underlyingMap["{$m}m"] = (float)$uRow['underlying'];
                    $underlyingTsMap["{$m}m"] = $uRow['ts'] ?? $tsPrev;
                }
            }
        }
// Fetch historical OI at each lookback
            ...
            foreach(['CE'=>$rowCE,'PE'=>$rowPE] as $opt=>$cur){
                ...
                foreach($lookbacks as $m){
                    $cutoff = (new \DateTime($latestTs,new \DateTimeZone('UTC')))
                                ->modify("-{$m} minutes")->format('Y-m-d H:i:s');
                    $prev = $db->query("
                        SELECT oi,chg_oi,vol,underlying
                        FROM oi_snapshots
        ...
        return $this->response->setJSON([
            ...
            'underlying_ts_map' => $underlyingTsMap,
