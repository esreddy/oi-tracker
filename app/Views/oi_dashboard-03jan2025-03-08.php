...
<div class="wrap">
  ...
  <div class="status-filter-row">
    ...
    <div class="status-top-right"><span class="pill warn" id="safetyBadge" style="display:none;" title="Safety Mode is ON: gap-based auto-pause is overridden so you can view last snapshots.">Safety: ON</span>
      <span class="pill">
...
async function loadDaywise(){
  ...
  const res = await fetch(`/oi/daywise?symbol=${encodeURIComponent(symbol)}&days=${encodeURIComponent(days)}&mode=${encodeURIComponent(mode)}`);
  const j = await res.json();
...
(function(){
  ...
  function updateLiveState(dbMins){
    ...
    // Auto-collapse Advanced Delta section after 15:30 IST
    try{
      const det = document.getElementById("advancedStrikeDeltas");
      const n = nowIST();
      const hh = String(n.getHours()).padStart(2,'0');
      const mm = String(n.getMinutes()).padStart(2,'0');
      const hhmm = Number(hh+mm);
      if(det && hhmm >= 1530) det.open = false;
    }catch(_){}

    // During CLOSED hours, don't scare with "FROZEN"
    if (st === "CLOSED") {
    ...
    const mins = (Number.isFinite(dbMins) ? Math.max(0, Math.round(dbMins)) : NaN);
    if (!Number.isFinite(mins)) {
      el.textContent = `${st} (age n/a)`;
      el.className = "market-preopen";
      return;
    }

    // Thresholds (minutes)
    const LIVE_M = 2;      // <= 2m live
    const DELAY_M = 5;     // 3-5m delayed
    // >5m frozen

    if (mins <= LIVE_M) {
      el.textContent = `LIVE (db age ${mins}m)`;
      el.className = "data-live";
    } else if (mins <= DELAY_M) {
      el.textContent = `DELAYED (db age ${mins}m)`;
      el.className = "market-preopen";
    } else {
      el.textContent = `FROZEN (db age ${mins}m)`;
