        ...
        <div style="padding:10px; border-radius:12px; border:1px solid rgba(255,255,255,.12); background:rgba(255,255,255,.04);">
          ...
          <div style="display:grid; grid-template-columns:70px 1fr; gap:6px 10px; margin-top:8px; font-size:13px;">
            <?php foreach (['day'=>'Day','week'=>'Week','d30'=>'30D','mtd'=>'MTD','year'=>'YTD'] as $k=>$label): ?>
              <div><?= $label ?></div>
              <div style="padding:2px 6px; border-radius:8px; background:<?= $intensityBg($m[$k]['pct'] ?? null) ?>;">
...
async function loadTrack(){
  ...
  head.innerHTML = `
    ...
    ${minsDesc
      ...
      .join('')}
    <th>OI @Open</th><th>ΔOI Day (from open)</th><th>Current OI</th><th>Cur ΔOI (chg & %)</th><th>Volume</th><th>Cur Vol (chg & %)</th>
  `;
  ...
  (j.rows || []).forEach(r => {
    ['CE','PE'].forEach(side=>{
      ...
      // Day-from-open delta (computed): uses controller's callDayDelta/putDayDelta arrays.
      const dayDeltaMap = (side==='CE') ? (j.callDayDelta||{}) : (j.putDayDelta||{});
      const dayDeltaVal = (dayDeltaMap && Object.prototype.hasOwnProperty.call(dayDeltaMap, r.strike))
        ? dayDeltaMap[r.strike]
        : null;

      const openOi = (dayDeltaVal===null || dayDeltaVal===undefined || curOi===null || curOi===undefined)
        ? null
        : (curOi - dayDeltaVal);

      const dayDeltaCell = (dayDeltaVal===null || dayDeltaVal===undefined)
        ? '-'
        : `<span class="cur-delta ${(dayDeltaVal>0)?'up':((dayDeltaVal<0)?'down':'')}">${dayDeltaVal>0?'▲ ': (dayDeltaVal<0?'▼ ':'')}${fmt(dayDeltaVal)}</span>`;
...
<?php if (isset($indexMoves) && is_array($indexMoves)): ?>

<div style="margin:24px 12px 12px 12px; padding:14px;
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
    ...
    <div class="muted" style="font-size:12px;">
      Daily / Weekly / Monthly (Close-to-Close)
    </div>
  </div>
  ...
  <?php
    ...
    $arrow = function($d){
      if ($d > 0) return '<span style="color:#22c55e;font-weight:800;">▲</span>';
      if ($d < 0) return '<span style="color:#ef4444;font-weight:800;">▼</span>';
      return '<span style="opacity:.6;">•</span>';
    };
  ?>
      ...
      <div style="padding:12px;
        ...
        <?php if (empty($m['ok'])): ?>
          ...
          <div class="muted" style="margin-top:8px;">
            No EOD data yet.<br>
            Run:
            <code>php spark index:eod:update</code>
          </div>
