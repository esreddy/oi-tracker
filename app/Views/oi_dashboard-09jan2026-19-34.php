...
headEl.innerHTML =
    ...
    // ===== Footer totals (FULL EXPIRY) =====
    const foot = document.getElementById('nseOcFoot');
    if(foot){
      const tnow = j?.expiry_totals?.now || null;
      if(tnow){
        const ceOiT = tnow.ce_oi;
        const peOiT = tnow.pe_oi;
        const ceVolT = tnow.ce_vol;
        const peVolT = tnow.pe_vol;
        ...
        foot.innerHTML = `
          <tr class="nse-foot">
            <td colspan="${leftSpan}" style="text-align:left;padding:10px 12px">
              <span class="muted">Total OI (Expiry):</span>
              <b style="margin-left:6px">CE ${fmt(ceOiT)}</b>
              <span class="muted" style="margin:0 8px">·</span>
              <b>PE ${fmt(peOiT)}</b>
              <span class="muted" style="margin:0 8px">·</span>
              <b>PCR ${(typeof pcr==='number') ? pcr.toFixed(2) : '—'}</b>
            </td>
            ...
            <td colspan="${rightSpan}" style="text-align:right;padding:10px 12px">
              ...
              <b style="margin-left:6px">CE ${ceVolT!==null?fmt(ceVolT):'—'}</b>
              <span class="muted" style="margin:0 8px">·</span>
              <b>PE ${peVolT!==null?fmt(peVolT):'—'}</b>
            </td>
          </tr>`;
      } else {
        foot.innerHTML = '';
