...
function buildNseMetricButtons(){
  ...
  function getDeltaForTf(sideSnap, tf, metric){
    ...
    return `<span class="dcell"><span class="arr ${arrCls}">${arr}</span><span class="num">${fmt(Math.abs(n))}</span>${pctHtml}</span>`;
  }
  ...
  window.renderNseOcFromTrack = function(j, _rerenderOnly){
    ...
    rows.forEach(r=>{
      const st = Number(r?.strike);
      const ceV = getDeltaForTf(r?.CE, __nseOcTf, __nseOcMetric);
      const peV = getDeltaForTf(r?.PE, __nseOcTf, __nseOcMetric);
    ...
    const _metLbl = (__nseOcMetric==='VOL') ? 'ΔVOL' : 'ΔOI';
    const thColsCalls = colsCalls.map(c=>{
      const label = (c==='DAY') ? (_metLbl+' Day') : (_metLbl+' ' + c + 'm');
...
headEl.innerHTML =
    ...
    rows.forEach(r=>{
      ...
      const ceChgCells = colsCalls.map(c=>{
        const v = getDeltaForTf(CE, c, __nseOcMetric);
                const cls = (v===null||v===undefined) ? '' : (String(c)==='DAY' ? (Number(v)>=0 ? 'pos':'neg') : '');
        const hi  = (String(c)===String(__nseOcTf) && Math.abs(Number(v))===maxCeAbsChg) ? 'max-chg' : '';
        const colhi = (String(c)===String(__nseOcTf)) ? ' col-hi' : '';
        return `<td class="${cls} ${hi}${colhi} ${ceItmCls}">${fmtDeltaCell(v, getPrevBaseForTf(CE, c, __nseOcMetric))}</td>`;
      }).join('');
      ...
      const peChgCells = colsPuts.map(c=>{
        ...
        const colhi = (String(c)===String(__nseOcTf)) ? ' col-hi' : '';
        return `<td class="${cls} ${hi}${colhi} ${peItmCls}">${fmtDeltaCell(v, getPrevBaseForTf(PE, c, __nseOcMetric))}</td>`;
      }).join('');

      html.push(
        `<tr class="${rowCls}">
          <td class="${ceOiCls} ${ceItmCls}">${fmt(ceOi)}</td>
          ${ceChgCells}
          <td class="${ceItmCls}">${fmt(ceVol)}</td>

          <td class="${strikeCls}">${fmt(st)}</td>

          <td class="${peItmCls}">${fmt(peVol)}</td>
          ${peChgCells}
          <td class="${peOiCls} ${peItmCls}">${fmt(peOi)}</td>
        </tr>`
      );
    });
    ...
    // ===== Footer totals (EXPIRY + WINDOW) =====
    const foot = document.getElementById('nseOcFoot');
    if(foot){
      // Footer layout matches current columns
      const leftSpan  = 1 + colsCalls.length + 1; // CE OI + CE Δ cols + CE VOL
      const rightSpan = 1 + colsPuts.length  + 1; // PE VOL + PE Δ cols + PE OI

      // Expiry totals (authoritative) from controller
      const exp = j?.expiry_totals?.now || j?.expiry_totals || null;

      // Window totals computed from currently displayed rows (ATM ±N)
      let wCeOi = 0, wPeOi = 0, wCeVol = 0, wPeVol = 0, wHasVol = false;
      rows.forEach(r=>{
        ...
        const ceV  = Number(r?.CE?.cur_vol); if(Number.isFinite(ceV)) { wCeVol += ceV; wHasVol = true; }
        const peV  = Number(r?.PE?.cur_vol); if(Number.isFinite(peV)) { wPeVol += peV; wHasVol = true; }
      });
      ...
      function lineHTML(label, ceOi, peOi, pcrVal){
        return `
          <div style="line-height:1.25">
            <span class="muted">${label}</span>
            <b style="margin-left:6px">CE ${fmt(ceOi)}</b>
            <span class="muted" style="margin:0 8px">·</span>
            <b>PE ${fmt(peOi)}</b>
            <span class="muted" style="margin:0 8px">·</span>
            <b>PCR ${(typeof pcrVal==='number') ? pcrVal.toFixed(2) : '—'}</b>
          </div>`;
      }

      function volLineHTML(label, ceVol, peVol, show){
        if(!show) return '';
        return `
          <div style="line-height:1.25">
            <span class="muted">${label}</span>
            <b style="margin-left:6px">CE ${Number.isFinite(ceVol)?fmt(ceVol):'—'}</b>
            <span class="muted" style="margin:0 8px">·</span>
            <b>PE ${Number.isFinite(peVol)?fmt(peVol):'—'}</b>
          </div>`;
      }
      ...
      if(exp && (Number.isFinite(+exp.ce_oi) || Number.isFinite(+exp.pe_oi))){
        ...
        const expCeVol = (typeof exp.ce_vol === 'number') ? exp.ce_vol : NaN;
        const expPeVol = (typeof exp.pe_vol === 'number') ? exp.pe_vol : NaN;

        const leftHtml =
          lineHTML('Total OI (Expiry):', expCe, expPe, expP) +
          (showWindow ? lineHTML(`Total OI (Window ATM ±${selStrikes}):`, wCeOi, wPeOi, wPcr) : '');

        const rightHtml =
          volLineHTML('Vol (Expiry):', expCeVol, expPeVol, Number.isFinite(expCeVol) || Number.isFinite(expPeVol)) +
          (showWindow ? volLineHTML('Vol (Window):', wCeVol, wPeVol, wHasVol) : '');

        foot.innerHTML = `
          <tr class="nse-foot">
            <td colspan="${leftSpan}" style="text-align:left;padding:10px 12px">
              ${leftHtml}
            </td>
            <td class="strike sticky-strike" style="font-weight:800;opacity:.9">TOTAL</td>
            <td colspan="${rightSpan}" style="text-align:right;padding:10px 12px">
              ${rightHtml || '<span class="muted">Vol:</span> <b style="margin-left:6px">—</b>'}
            </td>
          </tr>`;
      } else {
        // If controller totals are missing, show only window totals (and label it clearly)
        foot.innerHTML = `
          <tr class="nse-foot">
            <td colspan="${leftSpan}" style="text-align:left;padding:10px 12px">
              ${lineHTML(`Total OI (Window ATM ±${selStrikes}):`, wCeOi, wPeOi, wPcr)}
            </td>
            <td class="strike sticky-strike" style="font-weight:800;opacity:.9">TOTAL</td>
            <td colspan="${rightSpan}" style="text-align:right;padding:10px 12px">
              ${volLineHTML('Vol (Window):', wCeVol, wPeVol, wHasVol) || '<span class="muted">Vol:</span> <b style="margin-left:6px">—</b>'}
            </td>
          </tr>`;
      }
    }
