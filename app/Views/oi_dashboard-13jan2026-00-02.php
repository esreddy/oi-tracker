...
/* ATM “center marker” line like NSE (on the strike cell) */
#nseOcWrap tbody tr.atm-row td.strike,
#nseOcWrap tbody tr.atm-row td.col-strike,
#nseOcWrap tbody tr.atm-row td[data-col="strike"]{
  position: relative;
}
#nseOcWrap tbody tr.atm-row td.strike::after,
#nseOcWrap tbody tr.atm-row td.col-strike::after,
#nseOcWrap tbody tr.atm-row td[data-col="strike"]::after{
  content: "";
  position: absolute;
  top: -1px;
  bottom: -1px;
  ...
  background: rgba(250,204,21,.75); /* NSE-like yellow marker */
...
(function(){
  ...
  function highlightNseAtmRow(j){
    ...
    // Auto-scroll ATM row to center (only if table is visible)
    try{
      const wrap = document.getElementById('nseOcWrap') || body.parentElement;
      const rect = wrap?.getBoundingClientRect?.();
      const visible = rect && rect.bottom > 0 && rect.top < (window.innerHeight || 800);
      if(visible){
