...
function buildNseMetricButtons(){
  ...
  window.renderNseOcFromTrack = function(j, _rerenderOnly){
    ...
    const callSpan = 1 + colsCalls.length + 2; // Vol + TF cols + (Core + Change)
    const putSpan  = 2 + colsPuts.length + 1; // (Core + Change) + TF cols + Vol
...
// Core Δ (for the central Change column): ALWAYS DAY ΔOI (independent of metric / selected TF)
    ...
    rows.forEach(r=>{
      const ceV = Number(getDeltaForTf(r?.CE, 'DAY', __nseOcMetric));
      const peV = Number(getDeltaForTf(r?.PE, 'DAY', __nseOcMetric));
...
headEl.innerHTML =
    ...
    rows.forEach(r=>{
      ...
      const hintBadge = (hint==='BUY' || hint==='SELL')
        ? `<span class="oc-hint ${hint==='BUY'?'oc-hint-buy':'oc-hint-sell'}">${hint}</span>`
        : '';
      ...
      const ceTfCells = colsCalls.map((c,i)=>{
        ...
        return `<td class="${cls}${hi}${sep} ${ceItmCls}">${fmtDeltaCell(v, getPrevBaseForTf(CE, c, __nseOcMetric))}</td>`;
      }).join('');
      ...
      const peTfCells = colsPuts.map(c=>{
        ...
        const cls = ''; // keep numbers white; arrow shows direction
        return `<td class="${cls}${hi} ${peItmCls}">${fmtDeltaCell(v, getPrevBaseForTf(PE, c, __nseOcMetric))}</td>`;
      }).join('');
      ...
      const ceSelCell = `<td class="core-chg ${ceSelCls} ${ceSelHi} col-hi ${ceItmCls}">${fmtDeltaCell(ceSelV, getPrevBaseForTf(CE, 'DAY', 'OI'))}</td>`;
      const peSelCell = `<td class="core-chg ${peSelCls} ${peSelHi} col-hi ${peItmCls}">${fmtDeltaCell(peSelV, getPrevBaseForTf(PE, 'DAY', 'OI'))}</td>`;html.push(
        `<tr class="${rowCls}">
