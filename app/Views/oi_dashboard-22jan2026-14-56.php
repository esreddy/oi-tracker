    ...
    <div class="tb-main">
      ...
      <div class="snap-box" id="snapBox">
        ...
        <div class="snap-actions">
          <button class="pill" id="snapSave" title="Save a snapshot of the current NSE table (and key Market Meter stats)">📸 Save</button>
          <select id="snapSelect" class="pill" style="max-width:260px" title="Choose a saved snapshot"></select>
          <button class="pill" id="snapLoad" title="Load the selected snapshot into NSE table">Load</button>
          <button class="pill" id="snapQuick1130" title="1-click: Load snapshot closest to 11:30 AM">🕦 11:30</button>
          <button class="pill" id="snapQuickPreclose" title="1-click: Load snapshot closest to Pre-close (around 3:25–3:30 PM)">⏳ Pre-close</button>
          <label class="pill" style="display:inline-flex;align-items:center;gap:6px" title="Show quick compare badges (Live vs Replay) in the header">
            <input type="checkbox" id="snapCompare" style="transform:scale(1.05);margin:0"> Compare
          </label>
          <button class="pill" id="snapDelete" title="Delete selected snapshot">🗑</button>
        </div>
        <div class="snap-hint muted" id="snapHint">Tip: Save at Open / 11:30 / Pre-close. After 3:30 PM, Pre-close auto-loads once per session.</div>
      </div>
    </div>
...
(function(){
  ...
  if(btn){
    ...
    function snapParseKeyToMinutes(k){
      // key format: YYYY-MM-DD HH:MM:SS
      if(!k || typeof k!=='string') return null;
      const m = k.match(/\s(\d{2}):(\d{2})/);
      if(!m) return null;
      return (parseInt(m[1],10)*60) + parseInt(m[2],10);
    }
    function snapFindClosest(targetMin){
      const keys = Object.keys(snapshots||{});
      if(!keys.length) return null;
      let bestK=null, bestD=1e9;
      for(const k of keys){
        const mins = snapParseKeyToMinutes(k);
        if(mins===null) continue;
        const d = Math.abs(mins - targetMin);
        if(d < bestD){ bestD=d; bestK=k; }
      }
      return bestK;
    }
    function snapLoadKey(k, forceCompare){
      if(!k) return;
      sel.value = k;
      if(forceCompare && cmpChk) cmpChk.checked = true;
      loadBtn.click();
    }
...
(function(){
  ...
  function bindSnapshotUI(){
    ...
    const quick1130 = document.getElementById('snapQuick1130');
    const quickPreclose = document.getElementById('snapQuickPreclose');
    const cmpChk = document.getElementById('snapCompare');
    ...
    // Quick buttons
    if(quick1130){
      quick1130.addEventListener('click', ()=>{
        const k = snapFindClosest(11*60 + 30);
        if(k) snapLoadKey(k, true);
      });
    }
    if(quickPreclose){
      quickPreclose.addEventListener('click', ()=>{
        const k = snapFindClosest(15*60 + 25); // ~3:25 PM
        if(k) snapLoadKey(k, true);
      });
    }

    // Auto-load Pre-close once per session after 3:30 PM (if any snapshot exists)
    try{
      const now = new Date();
      const minsNow = now.getHours()*60 + now.getMinutes();
      if(minsNow >= (15*60+30) && !sessionStorage.getItem('snap_preclose_autoload_done')){
        const k = snapFindClosest(15*60 + 25);
        if(k){
          sessionStorage.setItem('snap_preclose_autoload_done', '1');
          snapLoadKey(k, true);
        } else {
          sessionStorage.setItem('snap_preclose_autoload_done', '1');
        }
    ...
    btn.addEventListener('click', ()=>{
      const srcJ = window.__nseOcLastJ
             || window.__nseOcPending
             || (window.__lastTrackJson && (window.__lastTrackJson.nse_oc || window.__lastTrackJson.nse || window.__lastTrackJson.nseOc))
             || null;

      // Normalize to NSE-OC payload
      const j = (srcJ && srcJ.rows) ? srcJ : (srcJ && (srcJ.nse_oc || srcJ.nse || srcJ.nseOc) && (srcJ.nse_oc || srcJ.nse || srcJ.nseOc).rows ? (srcJ.nse_oc || srcJ.nse || srcJ.nseOc) : null);

      // Trade plan is best-effort; do not block saving if plan can't be computed
      const plan = j ? (computeTradePlan(j) || {bias:'NEUTRAL', conf:0, net:0, imb:0}) : null;

      if(!j){

        const h = document.getElementById('snapHint');
        if(h){
          h.textContent = 'Cannot save (no NSE data yet)';
          setTimeout(()=>{
            h.textContent = 'Tip: Save at Open / 11:30 / Pre-close. After 3:30 PM, Pre-close auto-loads once per session.';
          }, 1600);
        }
        console.warn('Snapshot save skipped: NSE payload not ready', {hasJ: !!j});
        return;
      }
      const snaps = loadSnaps();
